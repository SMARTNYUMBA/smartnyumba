        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex flex-col items-center gap-2 py-3">
            ...
            {user.is_suspended
              ...
              : !user.is_active && ['property_manager','caretaker','security'].includes(user.role)
                ? <span className="badge badge-amber">Pending activation</span>
          ...
          <div className="space-y-2 pt-2">
            ...
            {['property_manager','caretaker','security'].includes(user.role) && (
              user.is_suspended
                ...
                : !user.is_active
                  ? <button className="w-full text-sm py-2 px-4 rounded-xl bg-[--green-bg] text-green-700 border border-[--green-bg] hover:bg-green-100 font-semibold" onClick={() => onUnsuspend(user)}>✅ Activate account</button>
...
// BUG FIX: this used to map over the full ROLES list, which includes
// 'super_admin' — but the backend's /users/invite endpoint has always
// rejected that role outright (400 "Invalid role"), by design: it's an
// email-based flow that puts a temporary password in the invite email,
// which isn't appropriate for the platform's highest-privilege role. The
// UI never reflected that restriction, so picking "Super Admin" here led
// to a confusing dead end. A super_admin CAN be created — via "Add new
// user" instead, which sets the password directly with no email step —
// so that's what this now points to.
const INVITABLE_ROLES = ROLES.filter(r => r !== 'super_admin');

function InviteModal({ open, onClose, props, onAddDirect }) {
      ...
      <div className="p-5 flex flex-col gap-3">
        ...
        <div className="form-group">
          ...
          <select className="input" value={role} onChange={e => { setRole(e.target.value); setPropId(''); }}>
            {INVITABLE_ROLES.map(r => <option key={r} value={r}>{roleName(r)}</option>)}
          </select>
        </div>
        {needsProp && (
          <div className="form-group">
        ...
        <p className="text-xs text-[--text-muted]">
          Need to add a Super Admin instead? Use{' '}
          <button type="button" className="underline hover:text-[--brand]" onClick={() => { onClose(); onAddDirect?.(); }}>
            Add new user
          </button>{' '}
          — Super Admin accounts are created with a password you set directly, not by email.
        </p>
      </div>
      <div className="px-5 pb-5 flex items-center justify-end gap-2">
        <button className="btn-secondary" onClick={onClose}>Cancel</button>
...
export default function Users() {
  ...
  return (
    <AppLayout
      ...
      {/* ── Invite modal ───────────────────────────────────────────────────────── */}
      <InviteModal open={modal === 'invite'} onClose={() => setModal(null)} props={props} onAddDirect={() => setModal('add')} />
